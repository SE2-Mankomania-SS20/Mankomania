...
public class SlotIconRow {
    ...
    public void render(Batch batch) {
        ...
        batch.draw(this.slotTextures.getSlotTextureFirstRow(), this.xPosition, this.yPosition + this.offsetHeight, WIDTH, HEIGHT);
        batch.draw(this.slotTextures.getSlotTextureSecondRow(), this.xPosition, this.yPosition + HEIGHT + this.offsetHeight, WIDTH, HEIGHT);
        batch.draw(this.slotTextures.getSlotTextureThirdRow(), this.xPosition, this.yPosition + HEIGHT * 2 + this.offsetHeight, WIDTH, HEIGHT);
        batch.draw(this.slotTextures.getSlotTextureDuplicatIcon(), this.xPosition, this.yPosition + HEIGHT * 3 + this.offsetHeight, WIDTH, WIDTH);
    }
    ...
    public void update(float delta) {
        ...
        if (!this.isStopping) {
            ...
            float percentage = Math.min(1f, this.interpolationCurrent / INTERPOLATION_MAX); // 0 -> 1, 0 = beginning, 1 = stopped
    ...
    public void stopAt(int iconIndex) {
        ...
        if (!this.isStopping && !this.shouldStop) {
            ...
            this.stopYPositionGoal = -(iconIndex * WIDTH);
            this.stopAtYPosition = this.stopYPositionGoal + WIDTH * 3;

            // if we are positive (meaning we scrolled out of bounds in our case), loop over
            if (this.stopAtYPosition > 0) {
                this.stopAtYPosition = -(WIDTH * 8) + this.stopAtYPosition;
            }
    ...
    private void checkOffsetOutOfBounds() {
        if (this.offsetHeight <= -(HEIGHT * 3)) {
            this.offsetHeight = this.offsetHeight + HEIGHT * 3;
        }
    }
    ...
    private void checkForStopping() {
        ...
        if (this.offsetHeight < this.stopAtYPosition + WIDTH / 2 && this.offsetHeight > this.stopAtYPosition - WIDTH / 2 && this.shouldStop) {
            ...
            if (this.stopYPositionGoal < this.offsetHeight) {
                ...
                this.stopDistance = (HEIGHT * 3 + this.offsetHeight) - this.stopYPositionGoal;
            }
        }
    }
